...
namespace BookLibraryAPIDemo.Application.Commands.BookLibraryAPIAuth
{
    public class Login : IRequest<object>
    {
        ...
        public class LoginHandler : IRequestHandler<Login, object>
        {
            ...
            {
                _user = await _userManager.FindByNameAsync(request.LoginDTO.Username);
                var result = (_user != null && await _userManager.CheckPasswordAsync(_user, request.LoginDTO.Password));

                if (!result)
                {
                    _logger.LogWarn($"{nameof(Login)}: Login failed. Wrong username or password");
                }
